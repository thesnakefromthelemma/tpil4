...
def fp_style :
  {α : Sort u} → {p : α → Prop} → {r : Prop} →
  (∃ x, p x ∧ r) ↔ (∃ x, p x) ∧ r
:=
  Iff.intro
    (fun (Exists.intro x (And.intro hpx hr)) => And.intro (Exists.intro x hpx) (hr))
    (fun (And.intro (Exists.intro x hpx) hr) => Exists.intro x (And.intro hpx hr))
...
:=
  Iff.intro
    (fun hap (Exists.intro x hnpx) => hnpx (hap x))
    (fun hnexnpx x => Classical.byContradiction
      (fun hnpx => hnexnpx (Exists.intro x hnpx)))
...
:=
  Iff.intro
    (fun (Exists.intro x hpx) haxnpx => (haxnpx x) hpx)
    (fun hnaxnpx => Classical.byContradiction
      (fun hnep => hnaxnpx (fun x hpx => hnep (Exists.intro x hpx))))
...
:=
  Iff.intro
    (fun hnap => Classical.byContradiction
      (fun hnexnpx => hnap (fun x => Classical.byContradiction
        (fun hnpx => hnexnpx (Exists.intro x hnpx)))))
    (fun (Exists.intro x hnpx) hap => hnpx (hap x))
...
/- Classical (is there some slicker approach??) -/
namespace ex10
...
:=
  fun a =>
    Iff.intro
      (fun (Exists.intro x hipxr) hap => hipxr (hap x))
      (fun hiapr => Classical.byCases
        (fun hap => Exists.intro a (fun _ => hiapr hap))
        (fun hnap => Classical.byContradiction
          fun hnexipxr => hnap (fun x => Classical.byContradiction
            (fun hnpx => hnexipxr (Exists.intro x (fun hpx => nomatch hnpx hpx))))))
...
:=
  Iff.intro
    (Classical.byCases
      (fun hr _ => Or.inr hr)
      (fun hnr haxopxr => Or.inl (fun x => match haxopxr x with
        | Or.inl hpx => hpx
        | Or.inr hr => nomatch hnr hr)))
    (fun
    | Or.inl hap, x => Or.inl (hap x)
